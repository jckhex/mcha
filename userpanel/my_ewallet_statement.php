...
a.info:hover span{   display:block;   position:absolute;   top:2em; left:-10em; width:45em;   border:1px solid #0cf;  background-color:#FEE2AB; color:#000;    text-align: left}
        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header"> Wallet Statement
</h1>  
                </div>
                <!-- /.col-lg-12 -->
            </div>
 ...
 $schid=$_SESSION['reg_id'];
		      ...
		      $sql= mysqli_query($conn ,"select * from tbl_paylife_wallet ");   
                 while($qrftch=mysqli_fetch_array($sql))           
              {
                ?>
              <tr class="even gradeC">
                                          <td> <?php echo $qrftch['id']?></td>
                                           <?php $tmptime1 = strtotime($qrftch['trans_date']);
										    $tmpDeadline1 = date("d M, Y", $tmptime1);
										    ?>
											
                                       <td class="date"><?php echo $tmpDeadline1 ?></td>
                                        
                                          <td><?php echo $qrftch['tran_note']?></td>
                                       
                                       <td><?php echo $qrftch['user_id']?></td>
                                       <td><?php echo $qrftch['credit']?></td>
                                       
                                       <td><?php echo $qrftch['debit']?></td>
                                       <td><?php echo $qrftch['balance']?></td>
